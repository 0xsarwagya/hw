...
const Shop: React.FC = () => {
  ...
  const backendProducts = selectedCollectionId
                ...
                {paginatedProducts.map((product, idx) => (
                  <div key={product.id} className="group relative">
                    <Link
                      ...
                      className="block"
                    >
                      <div className="w-full overflow-hidden rounded-lg bg-white mb-4 shadow-sm group-hover:shadow-md transition-shadow relative">
                        <LazyImage
                          src={product.image}
                          alt={product.name}
                          aspectRatio="9:16"
