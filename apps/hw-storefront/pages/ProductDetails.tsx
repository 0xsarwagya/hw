...
const ProductDetails: React.FC = () => {
  ...
  // Refetch product when slug changes
  React.useEffect(() => {
    if (slug) {
  ...
  }, [slug]); // Only depend on slug, not refetchProduct
    ...
    if (product) {
      ...
      const firstImage =
        ...
        product.images.length > 0
          ? product.images[0]
          : product.image || "";
      setActiveImage(firstImage); // Reset image on product change
    }
  ...
  }, [product.images, product.image, activeImage]);
          ...
          <div className="flex flex-col gap-2 mb-6">
            ...
            {product.pricelistPrices && product.pricelistPrices.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-2">
                {product.pricelistPrices.map((plPrice) => (
                  <div
