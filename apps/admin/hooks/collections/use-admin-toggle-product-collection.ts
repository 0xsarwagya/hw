...
export function useAdminToggleProductCollection(productId: string) {
  ...
  return {
    addToCollection: useApiMutation<
      { message: string; added: number; skipped: number },
      { collectionId: string }
    >({
      ...
      onSuccess: (data, variables) => {
        queryClient.invalidateQueries({
          queryKey: [
            endpoints.collections.products.list(variables.collectionId),
          ],
        });
        queryClient.invalidateQueries({
          queryKey: [endpoints.collections.detail(variables.collectionId)],
        });
        queryClient.invalidateQueries({
          queryKey: [endpoints.products.collections(productId)],
        });
    ...
    removeFromCollection: useApiMutation<
      { message: string },
      { collectionId: string }
    >({
      mutationFn: async ({ collectionId }) => {
