...
export const createProductFormSchema = z.object({
  ...
  hsnCode: z.string().max(50, "HSN code must not exceed 50 characters").optional(),
  status: z.enum(["draft", "active", "archived"]).optional(),
  categoryId: z.string().uuid("Category ID must be a valid UUID").optional(),
});

// Schema with transform for API (gstRate is number)
export const createProductSchema = createProductFormSchema.transform((data) => ({
  ...data,
...
export const updateProductSchema = updateProductFormSchema.transform((data) => ({
  ...data,
  gstRate: data.gstRate ? parseInt(data.gstRate, 10) : undefined,
...
export const createVariantFormSchema = z.object({
  ...
  inventory: z.number().int().min(0, "Inventory must be greater than or equal to 0").optional(),
  ...
  optionValueIds: z.array(z.string().uuid("Option value ID must be a valid UUID")).optional(),
});

// Schema with transform for API (dates are Date objects)
export const createVariantSchema = createVariantFormSchema.transform((data) => ({
  ...data,
...
export const updateVariantFormSchema = z.object({
  ...
  inventory: z.number().int().min(0, "Inventory must be greater than or equal to 0").optional(),
  size: z.string().max(50, "Size must not exceed 50 characters").optional().nullable(),
...
export const updateVariantSchema = updateVariantFormSchema.transform((data) => ({
  ...data,
  compareAtPrice: data.compareAtPrice === null ? undefined : data.compareAtPrice,
