...
export const createCollectionSchema = z.object({
  ...
  imageUrl: z.string().max(500, "Image URL must not exceed 500 characters").optional(),
});
...
export const addProductsToCollectionSchema = z.object({
  productIds: z.array(z.string().uuid("Product ID must be a valid UUID")).min(1, "At least one product ID is required"),
});
