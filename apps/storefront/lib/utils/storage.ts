 ...
 * Also syncs to cookie for server-side access
 */
export function getGuestSessionId(): string {
  ...
  // Sync to cookie for server-side access (Server Actions)
  ...
  if (!sessionCookie || sessionCookie.split("=")[1] !== sessionId) {
    ...
    // biome-ignore lint/suspicious/noDocumentCookie: Cookie Store API not widely supported, document.cookie needed for client-side syncing
